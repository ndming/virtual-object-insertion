...
prompts the location of the object interatively. It proceeds to outputs several 
...
programmatically inserts the shaded object to the target 2D image.
...
use outdated libraries which make it cumbersome to set up the right working 
environment.
...
If `--target` is ommitted, the script will insert the object to the `target.png` 
...
If this option is ommitted, the result is placed in the resource directory. 
